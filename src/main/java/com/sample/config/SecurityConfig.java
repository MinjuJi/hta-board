 ...
 *  - BCrypt는 레인보우 테이블 공격을 방지하기 위해서 salting과 key stretching을 적용한다.
 *  
 * SecurityFilterChain
 * 	- Spring Security에서 인증/인가 관련 보안작업을 처리하는 각종 필터들의 집합이다.
 *  - 개발자는 HttpSecurity 객체가 제공하는 메소드를 이용해서 SecurityFilterChain에 포함된 각종 필터들에 대한 설정을 커스텀 할 수 있다.
 *  - 개발자는 커스텀 설정이 적용된 SecurityFilterChain을 반환하는 메소드를 정의해서 각종 보안설정을 한다.
 *  
 * HttpSecurity
