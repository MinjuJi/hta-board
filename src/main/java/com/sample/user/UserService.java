 ...
 *  - UserRepository는 인터페이스이다.
 *  - userRepository 프로퍼티에는 UserRepository 인터페이스의 구현객체가 의존성 주입된다.
 *  - UserRepository 인터페이스의 구현 객체는 Spring Data JPA가 JPA 구현체를 이용해서 구현하고 스프링 컨테이너의 빈으로 등록한다.
 */
@Service
...
public class UserService {
	 ...
	 * @param id 사용자 기본키 값
	 * @return 조회된 사용자 정보가 포함된 User 객체, 만약 사용자 정보가 존재하지 않으면 NoSuchElemetException 예외를 던진다.
	 */
