 ...
 *  - 엔티티는 데이터의 집합이다. 보통 테이블에 저장되는 하나의 레코드(행)를 말한다.
 ...
 *  		- id 값을 따로 할당하지 않아도 데이터베이스가 자동으로 auto_increment를 사용하여 기본키값을 생성한다.
 *  		- MySQL, MariaDB, H2 데이터베이스에서 주로 사용한다.
 *  - SEQUENCE
 *  		- 별도의 Sequence 객체를 이용해서 일렬번호를 발행하고, 그 값을 기본키 값으로 사용하는 방식이다.
 ...
 * 
 * @Enumerated
 * 	- 이 어노테이션은 상수 데이터의 주요한 값 중에서 어떤 값을 엔티티에서 사용할 것인지 지정하는 어노테이션이다.
 *  - EnumType.STRING - 상수데이터의 이름을 사용한다.
 *  - EnumType.ORDINAL - 상수데이터의 순서(0 ,1 , ...)을 사용한다.
 */
@Entity
